...
def main(page: ft.Page) -> None:
    ...
    def on_command_click(phrase: str) -> None:
        ...
        show_message(page, "正在播放语音…")

        def run_tts() -> None:
            try:
                hit = tts_edge.play_phrase(phrase)
            except Exception as ex:
                async def show_err(e: BaseException = ex) -> None:
                    show_message(page, f"TTS 失败：{e}")

                page.run_task(show_err)
            else:
                print(f"[Edge TTS] 已播放 {'(缓存)' if hit else '(新生成)'}：{phrase!r}")

        page.run_thread(run_tts)
